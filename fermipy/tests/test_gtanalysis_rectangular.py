...
def create_pg1553_analysis(request, tmpdir_factory):
    ...
    url = 'https://raw.githubusercontent.com/XanAstia/fermipy-extra/asymmetric-roi/data/fermipy_test_pg1553.tar.gz'
    ...
    for f in ft2_files:
        url = 'https://raw.githubusercontent.com/XanAstia/fermipy-extra/blob/asymmetric-roi/data/ft2/%s' % f
...
def test_gtanalysis_load_roi(create_draco_analysis):
    ...
    assert_allclose(prefactor['value'] *
                    prefactor['scale'], 3.427e-13, rtol=1E-3)
    assert_allclose(index['value'] * index['scale'], -2.028, rtol=1E-3)
    assert_allclose(src['flux'], 5.3473e-10, rtol=1E-3)
    assert_allclose(src['flux_err'], 6.446e-11, rtol=1E-3)
    assert_allclose(src['eflux'], 1.353e-06, rtol=1E-3)
    assert_allclose(src['eflux_err'], 1.603e-07, rtol=1E-3)
    assert_allclose(src['ts'], 200.86, rtol=1E-3)
    assert_allclose(src['npred'], 169.037, rtol=1E-3)
...
def test_gtanalysis_find_sources(create_draco_analysis):
    ...
    assert(flux_diff0 < 13.0)
    assert(flux_diff1 < 10.0)
...
def test_gtanalysis_localization(create_draco_analysis):
    ...
    assert_allclose(result['glon'], 86.026, atol=0.02)
    assert_allclose(result['glat'], 35.945, atol=0.02)
...
def test_gtanalysis_lightcurve(create_pg1553_analysis):
    ...
    rtol = 0.01
    flux = np.array([2.855591e-08,
                     2.260632e-08])
    flux_err = np.array([1.918914e-09,
			   1.790286e-09])
    #flux = np.array([2.860243e-08,
    #                 2.312212e-08])
    #flux_err = np.array([1.874528e-09,
    #                     1.762850e-09])
    ts = np.array([1446.526976,
                   1062.293046])
